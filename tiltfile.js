...
// src/getCachedConfig.ts
import { promises as fs } from "fs";
import path from "path";
async function getCachedConfig(filePath, initialTiltState) {
...
class TiltConfig {
  ...
  constructor() {
    ...
    this.state = {
      ...
      k8s: {
        ...
        namespace: this.normalizeNamespace("eco_test")
      },
      docker_build: {},
      k8s_yaml: {}
    };
  ...
  normalizeNamespace(namespace) {
    let normalized = namespace.toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/^-+|-+$/g, "").replace(/-+/g, "-");
  ...
  async init() {
    ...
    this.state.k8s.namespace = this.normalizeNamespace(this.state.k8s.namespace);
    this.initialState = structuredClone(this.state);
  ...
  getConfigFilePath() {
    ...
    return path2.join(process.cwd(), ".tilt-ts", `state-${PORT}.json`);
  }
  async writeToDisk() {
    await this.ensureInitialized();
    try {
      const dirPath = path2.dirname(this.configFilePath);
      await fs2.mkdir(dirPath, { recursive: true });
      await fs2.writeFile(this.configFilePath, JSON.stringify(this.state, null, 2), "utf8");
    } catch (error) {
      console.error(`❌ Failed to write config to ${this.configFilePath}:`, error);
      throw error;
    }
  }
  addDockerBuild(imageName, buildContext, hot) {
  ...
  async setK8sNamespace(namespace) {
    await this.ensureInitialized();
  ...
  async setDockerRegistry(registry) {
    await this.ensureInitialized();
    if (!registry || registry.trim().length === 0) {
      throw new Error("Docker registry cannot be empty");
    }
    const registryPattern = /^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*(:[0-9]+)?$/;
    if (!registryPattern.test(registry)) {
      console.warn(`⚠️  Registry "${registry}" may not be in correct format (host:port)`);
    }
  ...
  async configExists() {
    try {
      await fs2.access(this.configFilePath);
      return true;
    } catch {
      return false;
    }
  ...
  async deleteConfig() {
    try {
      await fs2.unlink(this.configFilePath);
...
function resolveYamlInput(input) {
  try {
    const normalizedInput = path4.resolve(input);
...
function containsGlobPattern(path5) {
  return /[*?[\]{}]/.test(path5) || path5.includes("**");
}
...
function resolveDirectory(dirPath) {
  try {
    const patterns = [
      path4.join(dirPath, "*.yaml"),
      path4.join(dirPath, "*.yml"),
      path4.join(dirPath, "**/*.yaml"),
      path4.join(dirPath, "**/*.yml")
    ];
    const allFiles = [];
...
function isDirectory(path5) {
  try {
    const stat = __require("fs").statSync(path5);
...
function isYamlFile(filePath) {
  const ext2 = path4.extname(filePath).toLowerCase();
  return ext2 === ".yaml" || ext2 === ".yml";
}

// src/docker_build.ts
function docker_build(imageName, buildContext = {
  context: ".",
  src: ["Dockerfile"]
}, hot) {
...
var run = (fileOrPath, options) => {
  return { type: "run", path: fileOrPath, options };
};

// tiltfile.ts
docker_build("ecosystem/nginx", {
  context: "./example",
  src: ["Dockerfile"]
}, {
  ignore: ["*.log", "tmp/*"],
  live_update: [
    sync2("example/*", "/usr/share/nginx/html")
  ]
});
k8s_yaml("./example/");
docker_build("ecosystem/app-test", {
  context: "./example",
  src: ["Dockerfile"]
}, {
