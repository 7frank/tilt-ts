...
export class DockerManager {
  ...
  async checkConnection(): Promise<void> {
    ...
    try {
      ...
      const pingResult = await ShellExecutor.execQuiet("docker", ["info"], {
        timeout: 10000,
      });
      ...
      const versionResult = await ShellExecutor.execQuiet("docker", [
        "version",
        "--format",
        "json",
      ]);
    ...
    try {
      ...
      try {
        ...
        if (!contextStat.isDirectory()) {
          throw new Error(`Build context is not a directory: ${contextPath}`);
        }
      } catch (error: any) {
      ...
      const dockerfileFullPath = path.join(contextPath, dockerfilePath);
      try {
        const dockerfileStat = await fs.stat(dockerfileFullPath);
        if (!dockerfileStat.isFile()) {
          throw new Error(`Dockerfile is not a file: ${dockerfileFullPath}`);
        }
      } catch (error: any) {
        if (error.code === "ENOENT") {
      ...
      const tagResult = await ShellExecutor.exec("docker", [
        "tag",
        imageName,
        privateTag,
      ]);
  ...
  private async pushImageWithRetry(
    imageTag: string,
    maxRetries: number
  ): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `📤 Pushing ${imageTag} (attempt ${attempt}/${maxRetries})`
        );

        // Use docker CLI for pushing (more reliable progress reporting)
        const pushResult = await ShellExecutor.execStream(
          "docker",
          ["push", imageTag],
          {
    ...
    throw new Error(
      `Failed to push after ${maxRetries} attempts: ${lastError?.message}`
    );
  }
  ...
  async removeImage(imageName: string): Promise<void> {
    try {
      ...
      for (const image of images) {
        const removeResult = await ShellExecutor.execQuiet("docker", [
          "rmi",
          image,
          "--force",
        ]);
  ...
  async getSystemInfo(): Promise<any> {
    try {
      ...
      const infoResult = await ShellExecutor.execQuiet("docker", [
        "system",
        "info",
        "--format",
        "json",
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      if (pruneDanglingResult.success) {
        ...
        console.warn(
          `   ⚠️  Failed to remove dangling images: ${pruneDanglingResult.stderr}`
        );
      }
      ...
      if (pruneBuildResult.success) {
        ...
        console.warn(
          `   ⚠️  Failed to clean build cache: ${pruneBuildResult.stderr}`
        );
      }
  ...
  async checkRegistryConnection(): Promise<boolean> {
    try {
      ...
      const canConnect = await ShellExecutor.testConnection(
        registryHost,
        registryPort,
        5000
      );
      ...
      const testResult = await ShellExecutor.execQuiet(
        "docker",
   ...
   */
  async getRunningContainers(): Promise<
    Array<{ name: string; image: string; status: string }>
  > {
    try {
  ...
  async stopContainersByImage(imageName: string): Promise<void> {
    try {
      ...
      if (containerIds.length > 0) {
        console.log(
          `🛑 Stopping ${containerIds.length} container(s) for image ${imageName}`
        );
